...
class TIDEV4(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            self.test_num_classes = len(batched_inputs[0]["category_names"]) if batched_inputs[0]["category_names"] is not None else 0
        ...
        if not self.training and names[0] is not None:
            if last_state == "train":
        ...
        (
            ...
            class_embed=visual_embed,
        ) if visual_text_switcher == 0 else self.transformer(
            multi_level_feats,
            multi_level_masks,
            multi_level_position_embeddings,
            query_embeds,
        ...
        for lvl in range(inter_states.shape[0]):
            ...
            outputs_class = self.class_embed[lvl](inter_states[lvl], visual_embed) if visual_text_switcher == 0 else self.class_embed[lvl](inter_states[lvl], text_embed)
        ...
        ](enc_state, visual_embed) if visual_text_switcher == 0 else self.transformer.decoder.class_embed[
            self.transformer.decoder.num_layers
    ...
    def preprocess_visual_prompt(self, batched_inputs):
        prompt_boxes = [self.visual_prompt_encoder._embed_boxes(x["instances"].gt_boxes.tensor, (x['instances'].image_height, x['instances'].image_width)) for x in batched_inputs]
        prompt_boxes = torch.stack(prompt_boxes)

        # Fill prompt_boxes to 80 boxes with coordinates outside gt boxes area
        for i, input_data in enumerate(batched_inputs):
            gt_boxes = input_data["instances"].gt_boxes.tensor
            image_height, image_width = input_data['instances'].image_height, input_data['instances'].image_width
            num_boxes_to_add = 80 - prompt_boxes[i].shape[0]
            if num_boxes_to_add > 0:
                additional_boxes = []
                for _ in range(num_boxes_to_add):
                    while True:
                        x1 = torch.randint(0, image_width, (1,))
                        y1 = torch.randint(0, image_height, (1,))
                        x2 = torch.randint(x1 + 1, image_width + 1, (1,))
                        y2 = torch.randint(y1 + 1, image_height + 1, (1,))
                        new_box = torch.tensor([x1, y1, x2, y2], dtype=torch.float32)
                        # Check if the new box is outside all gt boxes
                        if not any((new_box[0] < gt_box[2] and new_box[2] > gt_box[0] and new_box[1] < gt_box[3] and new_box[3] > gt_box[1]) for gt_box in gt_boxes):
                            additional_boxes.append(new_box)
                            break
                additional_boxes = torch.stack(additional_boxes)
                prompt_boxes[i] = torch.cat((prompt_boxes[i], additional_boxes), dim=0)
        return prompt_boxes
    ...
    def preprocess_image_and_name(self, batched_inputs):
        ...
        if category_names[0] is not None:
