...
from projects.ovdino.modeling.feature_enhancer import ConvNormLayer, RepVggBlock,CSPRepLayer
...
class GatedTransformerLayer(nn.Module):
    ...
    Transformer Architecture <https://arxiv.org/abs/2002.04745>`_ .con
...
class TIDETransformerEncoder(TransformerLayerSequence):
    def __init__(
        self,
        embed_dim: int = 256,
        num_heads: int = 8,
        ...
        super(TIDETransformerEncoder, self).__init__(
...
class TIDETransformerDecoder(TransformerLayerSequence):
    def __init__(
        self,
        embed_dim: int = 256,
        num_heads: int = 8,
        ...
        super(TIDETransformerDecoder, self).__init__(
...
class TIDETransformer(nn.Module):
    ...
    def __init__(
        ...
        hidden_dim = self.embed_dim  # Define hidden_dim based on embed_dim
    ...
    def _get_encoder_output(self, feats):
    ...
    def forward(
        ...
        **kwargs,
        ...
        feat_flatten = feat_flatten[-1]#torch.cat(feat_flatten, 1)
        mask_flatten_ = mask_flatten[-1]#torch.cat(mask_flatten, 1)
        lvl_pos_embed_flatten = lvl_pos_embed_flatten[-1]#torch.cat(lvl_pos_embed_flatten, 1)
        ...
        memory = self.encoder(
            ...
            query_key_padding_mask=mask_flatten_,
        ...
        proj_feats[2] = memory.permute(0, 2, 1).reshape(-1, self.embed_dim, h, w).contiguous()
